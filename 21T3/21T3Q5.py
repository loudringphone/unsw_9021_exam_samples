...
def betrothed_pair(n):
    ...
    for i in range(n, 10_000):
        m = sum(get_all_divisor(i)) - 1
        if i == sum(get_all_divisor(m)) - 1:
            print(f'The least number >= {n} that is the first member of a Betrothed pair is {i}')
            print(f'The Betrothed itself is ({i}, {m})')
            return
    # REPLACE pass ABOVE WITH YOUR CODE

    # POSSIBLY DEFINE OTHER FUNCTIONS
def get_all_divisor(num):
    proper_divisors = []
