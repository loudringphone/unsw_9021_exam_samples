...
4321, 12331, 12421, 13321]
    '''
    number = [int(chr) for chr in list(str(number))]
    subnbs = set()

    def add_digits(digits, next_index):
        for i in range(next_index, len(number)):
            digit = number[i]
            if sum(digits) + digit < sum_of_digits and i != len(number)-1:
                add_digits(digits + [digit], i+1)
            elif sum(digits) + digit == sum_of_digits:
                subnbs.add(int(''.join([str(num) for num in digits])+str(digit)))

    for i, d in enumerate(number):
        if i != len(number) - 1 and d < sum_of_digits:
            add_digits([d], i+1)
        if d == sum_of_digits:
            subnbs.add(d)
    subnbs = sorted(list(subnbs))
    return subnbs
