...
def number_of_words_in_dictionary(word_1, word_2):
    ...
    with open(dictionary_file) as file:
        content = [word.rstrip() for word in file if word.rstrip()]
        try:
            index_1 = content.index(word_1)
            index_2 = content.index(word_2)
            if index_1 == index_2:
                print(f'{word_1} is in dictionary.')
            else:
                word_count = abs(index_2 - index_1) + 1
                print(f'Found {word_count} words between {word_1} and {word_2} in dictionary.')
        except ValueError:
            if word_1 == word_2:
